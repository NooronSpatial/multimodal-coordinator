...
| `bakeoff install-size` | what a model costs to download, asked BEFORE a byte moves (4x, AC-245/AC-246; ONE request since 5a) | `--repo=` |
| `bakeoff downloads` | what a download costs: every engine fetched and deleted for real through the one downloader — size before the tap, wall-clock, MB/s, delete time (5a, AC-301, INSTRUMENTS §70). Lands in a scratch root under the temporary directory and removes it afterwards, so nothing a person is using is touched | `--mind=` (the 0.6B by default), `--voice` (include the 1.1 GB neural voice), `--keep` (leave the scratch root) |
