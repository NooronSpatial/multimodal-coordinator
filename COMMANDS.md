...
| `bakeoff tool-contract` | what the parameters cost in the prompt (three tables, the slope per spec character) and whether the number the person said is the number the tool gets — twenty sentences, three tools, six verdicts (4z, AC-276's Mac half; the phone decides) | `--model=` (defaults to the 0.6B; pass the 4B for the phone's model), `--runs=` (3), `--system=` (none by default; the 4B calls under one) |
